...
- [x] **安全性と安定性**:
    - [x] **無限ループ防止**: `max_iterations` によるエージェント暴走の抑止。
    - [x] **APIクォータ対策**: リトライロジックと待機時間の調整。

### フェーズ 2: 設計のエキスパート "Architect" (Design Phase)
- [x] **Architect Agent 実装**: 専門設計エージェントのクラス作成。
- [x] **要件定義スキル**: ユーザーの曖昧な指示から「何を」「どう」作るかを定義するプロンプト。
- [ ] **構造設計スキル**: 
    - `propose_structure(tree)`: ディレクトリ構成を提案するツール。
    - `define_tech_stack(stack)`: 使用技術を決定するツール。
- [x] **Managerとの連携**: 設計書（Markdown等）を成果物としてManagerに渡すフロー。

### フェーズ 3: マネージャーの「管理・采配」 (Management Phase)
- [x] **タスク分解の実装**: Architectの設計書やユーザー要件をLLMで解析し、実装タスクに分割する仕組み。
- [x] **アサイン機能**: サブエージェントを登録・管理する仕組み。
- [x] **相談ハブ機能**: エージェント間でタスク依頼や相談を仲介する機能 (`delegate_task`)。
...
- [x] **Architect用ツール**: 設計書保存などのファイル操作ツール実装 (`write_design_doc`, `list_project_files`)。
    - 実装 -> 疑問 -> 質問 -> 回答 -> 実装再開 のループ。

### フェーズ 5: 「実行と検証」 (Execution Phase)
- [ ] **Sandbox環境の強化**: コンテナ内でのコード実行、エラー検知と修正。
- [ ] **フルサイクル検証**: ユーザーの要求から完成までの一連のフロー。
    1.  **User**: 「TODOアプリ作りたい」
